...
- (void)setType:(NSInteger)type
{
    _type = type;
    if (_type == 1) {
        _navgationTabBar.yzWidth = kScreenWidth - 160;
    }
}
...
- (void)viewConfig
{
    ...
    _navgationTabBar = [[UIScrollView alloc] initWithFrame:self.bounds];
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        //        button.frame = CGRectMake(kScreenWidth/self.itemTitles.count*index, DOT_COORDINATE, kScreenWidth/self.itemTitles.count, scNavHeight);
        ...
        [button setTitleColor:_navTabBarNormalColor?:[UIColor whiteColor] forState:UIControlStateNormal];
        [button setTitleColor:_navTabBarSelectColor?:[UIColor redColor] forState:UIControlStateSelected];
...
{
    _scrollX = scrollX;
    CGFloat width = kScreenWidth/self.itemTitles.count;
    CGFloat yu = (scrollX - kScreenWidth*(int)(scrollX/kScreenWidth))/self.itemTitles.count ;
    if (yu < width/2) {
        self.currentItemIndex = scrollX/kScreenWidth;
    }else
    {
        self.currentItemIndex = scrollX/kScreenWidth + 1;
    }
    UIButton *button = _items[self.currentItemIndex];
    _line.frame = CGRectMake(button.frame.origin.x + 2.0f, _line.frame.origin.y, [_itemsWidth[self.currentItemIndex] floatValue] - 4.0f, _line.frame.size.height);
}
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    if (titles.count <= 5) {
        NSMutableArray *old = [@[] mutableCopy];
        ...
        for (NSString *title in titles)
        {
            ...
            NSNumber *width = [NSNumber numberWithFloat:adjustedSize.width ];
            [old addObject:width];
        }
        CGFloat total = 0;
        for (NSNumber *num in old) {
            total = total + [num floatValue];
        }
        if (total<kScreenWidth) {
            CGFloat space = ((kScreenWidth - total)/titles.count);
            ...
            for (NSString *title in titles)
            {
                ...
                NSNumber *width = [NSNumber numberWithFloat:adjustedSize.width + space];
...
//            self->_popView.hidden = !self->_popView.hidden;
//            self->_arrowButton.transform = CGAffineTransformIdentity;
//        } completion:^(BOOL finished) {
//            self->_navgationTabBar.hidden = !self->_navgationTabBar.hidden;
//            self->_arrowButton.layer.shadowRadius = 4.0f;
//            self->_arrowButton.layer.shadowOpacity = 20.0f;
//            self->_arrowButton.layer.shadowOffset = CGSizeMake(0, -5);
//        }];
//    }
}
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        self->_line.frame = CGRectMake(button.frame.origin.x + 2.0f, self->_line.frame.origin.y, [self->_itemsWidth[currentItemIndex] floatValue] - 4.0f, self->_line.frame.size.height);
    }];
}
