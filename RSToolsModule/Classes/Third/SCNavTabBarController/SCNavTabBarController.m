...
- (void)viewDidLoad
{
    ...
    [self initConfig];
    [self viewConfig];
    [_mainView setContentOffset:CGPointMake(_currentIndex * SCREEN_WIDTH, DOT_COORDINATE) animated:YES];
    if (self.loadDataBlock) {
        self.loadDataBlock(_currentIndex);
    }
}
...
- (void)viewInit
{
    ...
    _navTabBar.navTabBarNormalColor = _navTabBarNormalColor;
    _navTabBar.navTabBarSelectColor = _navTabBarSelectColor;
    _navTabBar.itemTitles = _titles;
    _navTabBar.arrowImage = _navTabBarArrowImage;
    _navTabBar.titleFont = _navTabBarFont;
    [_navTabBar updateData];
    
    _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, _navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - _navTabBar.frame.origin.y - _navTabBar.frame.size.height - kStatusBarHeight - NAVIGATION_BAR_HEIGHT)];
...
{
    _navTabBar.scrollX = scrollView.contentOffset.x;
...
//    [self subControllerLoadData:_currentIndex];
...
{
    [_mainView setContentOffset:CGPointMake(index * SCREEN_WIDTH, DOT_COORDINATE) animated:YES];
    if (self.loadDataBlock) {
        self.loadDataBlock(index);
    }
}
