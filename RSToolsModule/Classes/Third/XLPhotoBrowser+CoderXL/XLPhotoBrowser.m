...
- (void)iniaialUI
{
    ...
    if (self.currentImageIndex == 0) { // 如果刚进入的时候是0,不会调用scrollViewDidScroll:方法,不会展示第一张图片
        XLFormatLog(@"self.currentImageIndex == %ld",(long)self.currentImageIndex);
        [self showPhotos];
    }
...
- (NSURL *)highQualityImageURLForIndex:(NSInteger)index
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(photoBrowser:highQualityImageURLForIndex:)]) {
        ...
        if (!url) {
            XLPBLog(@"高清大图URL数据 为空,请检查代码 , 图片索引:%ld",(long)index);
            return nil;
        }
        ...
        if (![url isKindOfClass:[NSURL class]]) {
            ...
            XLPBLog(@"高清大图URL数据有问题,不是NSString也不是NSURL , 错误数据:%@ , 图片索引:%ld",url,(long)index);
            return nil;
        }
        return url;
    } else if(self.images.count>index) {
